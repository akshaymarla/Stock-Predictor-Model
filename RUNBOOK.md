...
```bash
# run from repo root, not src/ -- PYTHONPATH must include src/ for the
# core.db / cross-folder imports to resolve (run_historical_seed.sh sets
...
python3 src/fundamentals/fetch_shareholding_pattern.py
python3 src/fundamentals/fetch_financial_results.py
python3 src/fundamentals/fetch_balance_sheet.py
python3 src/fundamentals/fetch_cash_flow.py
python3 src/fundamentals/fetch_ratios.py
```

(Or just run `./run_historical_seed.sh`, which does all of the above in
the right order with per-step logging — see repo root.)
