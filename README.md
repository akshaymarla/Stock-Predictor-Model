...
> **2026-07-17 — restructured + migrated to Supabase.** `src/` is now split
> into stage subfolders (`core/`, `metadata/`, `prices/`, `risk/`,
> `events/`, `fundamentals/`, `macro/`, `derived/`) and the DB is Postgres
> (Supabase) instead of local SQLite. See `DB_FILE_MAP.md` for the new
> file → table map and `SUPABASE_DEPLOY.md` for setup. Table names,
> columns, and script CLI flags are unchanged — only file locations and
> the connection layer (`core/db.py`) moved.
