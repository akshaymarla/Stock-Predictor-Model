...
class stockScreener():
    ...
    endpoints = {"base" : "company/{token}/{consolidated}/",
                 "chart" : "/api/company/{screenerID}/chart/?q=Price-DMA50-DMA200-Volume&days=10000{consolidated}", #&consolidated=true
                 "quarterlyReport" : {"Sales" : "/api/company/{screenerID}/schedules/?parent=Sales&section=quarters{consolidated}", #&consolidated=
                 ...
                 "pnlReport" : {"Sales" : "/api/company/{screenerID}/schedules/?parent=Sales&section=profit-loss{consolidated}",
                 ...
                 "balanceSheet" : {"Borrowing" : "/api/company/{screenerID}/schedules/?parent=Borrowings&section=balance-sheet{consolidated}",
                 ...
                 "cashFlow" : {"OperatingAct" : "/api/company/{screenerID}/schedules/?parent=Cash+from+Operating+Activity&section=cash-flow{consolidated}",
                 ...
                 "shareHolding" : {"Promoters" : "/api/3/{screenerID}/investors/promoters/{duration}/", #duration #yearly
        ...
        for tag in tags :
            try :
                if section == "shareHolding":
                    url = self.baseurl + self.endpoints[section][tag].format(screenerID = self.screenerID, duration=duration)
                    pass
                else:
    ...
    def __corporateAnnouncements(self, prevDate, toDate):
        ...
        all_data = []
        url = self.bseBaseurl + self.endpoints['corporateAnnouncements'].format(pageno = 1,
                                                                                prevDate=prevDate,
                                                                                toDate=toDate,
                                                                                token=self.token)
        ...
        if rows >= 50 :
            ...
            for i in range(sets):
                pageno = i + 2
                url = self.bseBaseurl + self.endpoints['corporateAnnouncements'].format(pageno = pageno,
                                                                                        prevDate=prevDate,
                                                                                        toDate=toDate,
                                                                                        token=self.token)
                data = self.requestBSE(url, "GET")
...
class ScreenerScrape(stockScreener):
    ...
    def __init__(self):
        try:
            self.tokendf = pd.read_csv(r"./tokens/tokens_{dt}.csv".format(dt=datetime.datetime.now().strftime('%Y%m%d')))
        except:
